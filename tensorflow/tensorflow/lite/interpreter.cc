...
#define LOGV(...) __android_log_print(ANDROID_LOG_VERBOSE, "interpreter", __VA_ARGS__) 
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG , "interpreter", __VA_ARGS__)
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO  , "interpreter", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN  , "interpreter", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR  , "interpreter", __VA_ARGS__)
...
Interpreter::Interpreter(ErrorReporter* error_reporter)
  ...
  TFLITE_LOG_PROD_ONCE(TFLITE_LOG_INFO, "mt1:Initialized TensorFlow Lite runtime.");
#else
  TFLITE_LOG_ONCE(TFLITE_LOG_INFO, "mt2:Initialized TensorFlow Lite runtime.");
#endif
  //if(remote_session_control) {
    //struct remote_rpc_control_unsigned_module data;
    //data.enable = 1;
    //data.domain = CDSP_DOMAIN_ID;
    //LOGI("mt---%s---%s",CDSP_DOMAIN_ID,data.domain);
...
void Interpreter::AddSubgraphs(int subgraphs_to_add,
  ...
  for (int i = 0; i < subgraphs_to_add; ++i) {
    ...
    LOGI("mt%d---subgraph_size%d",i,subgraph->nodes_size());
  }
}
