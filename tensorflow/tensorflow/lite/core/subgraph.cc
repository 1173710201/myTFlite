...
#define LOGV(...) __android_log_print(ANDROID_LOG_VERBOSE, "subgraph", __VA_ARGS__) 
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG , "subgraph", __VA_ARGS__)
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO  , "subgraph", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN  , "subgraph", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR  , "subgraph", __VA_ARGS__)
namespace tflite {
...
TfLiteStatus Subgraph::ReplaceNodeSubsetsWithDelegateKernels(
  ...
  //TfLiteIntArray* nodes_to_replace=TfLiteIntArrayCreate(3);
  //int data[]={10,11,12};
  //memcpy(nodes_to_replace->data,data,3*sizeof(int));
  LOGI("mt%d---%d---%d---%d",nodes_to_replace->size,nodes_to_replace->data[0],nodes_to_replace->data[1],nodes_to_replace->data[2]);
  ...
  PartitionGraphIntoIndependentNodeSubsets(&info, nodes_to_replace,
                                           &node_subsets);
  LOGI("mt---Replacing %d node(s) with delegate (%s) node",
...
TfLiteStatus Subgraph::Invoke() {
  ...
  for (int execution_plan_index = 0;
    ...
    if (profiler_) TFLITE_SCOPED_TAGGED_OPERATOR_PROFILE(profiler_.get(), op_name, node_index);
    ...
    //LOGI("context change time=%lld microseconds",t.count());
    ...
    //LOGI("index=%d op_name=%s start_time=%lld microseconds",node_index,op_name,start);
    ...
    LOGI("index=%d op_name=%s time=%lld microseconds",node_index,op_name,tt.count());
    //LOGI("mt : op_name=%s ",op_name);
    //ReportError("mt : op_name=%s time=%s microseconds",op_name,tt.count());
    //TFLITE_LOG_PROD_ONCE(TFLITE_LOG_INFO, "mt1:op_name:%s",op_name);
    //TFLITE_LOG_PROD_ONCE(
      //tflite::TFLITE_LOG_INFO,
      //"mt2 : op_name=%s time=%s microseconds",op_name,tt.count());
