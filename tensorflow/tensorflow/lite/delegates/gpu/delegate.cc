...
#endif
#define LOGV(...) __android_log_print(ANDROID_LOG_VERBOSE, "gpu_delegate", __VA_ARGS__) 
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG , "gpu_delegate", __VA_ARGS__)
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO  , "gpu_delegate", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN  , "gpu_delegate", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR  , "gpu_delegate", __VA_ARGS__)
namespace tflite {
namespace gpu {
namespace {
 ...
 public:
  explicit Delegate(const TfLiteGpuDelegateOptionsV2* options)
    ...
    if (options_.max_delegated_partitions <= 0) {
      options_.max_delegated_partitions = 300;
    }
    //options_.max_delegated_partitions = 2;
    LOGI("mt---gpuPartitions---%d",options_.max_delegated_partitions);
  }
...
class DelegateKernel {
 public:
  ...
  absl::Status Invoke(TfLiteContext* context) {
    ...
    auto tt = std::chrono::duration_cast<std::chrono::microseconds>(e - s);
    LOGI("---GPU dequantizeInputs time=%lld microseconds",tt.count());
    RETURN_IF_ERROR(SetInputsAndOutputs(context));
    auto s0=std::chrono::steady_clock::now();
    RETURN_IF_ERROR(runner_->Run());
    auto e0=std::chrono::steady_clock::now();
    auto tt0 = std::chrono::duration_cast<std::chrono::microseconds>(e0 - s0);
    LOGI("---GPU run time=%lld microseconds",tt0.count());
    ...
    auto tt1 = std::chrono::duration_cast<std::chrono::microseconds>(e1 - s1);
    LOGI("---GPU QuantizeOutputs time=%lld microseconds",tt1.count());
    return absl::OkStatus();
  }
...
TfLiteStatus DelegatePrepare(TfLiteContext* context, TfLiteDelegate* delegate) {
  ...
  LOGI("mtgpu---%d",ops_to_replace->size);
  TfLiteIntArrayFree(ops_to_replace);
  TfLiteIntArray* op_to_replace=TfLiteIntArrayCreate(6);
  int data[]={10,11,12,20,21,22};
  memcpy(op_to_replace->data,data,6*sizeof(int));
  LOGI("mtgpu---%d",op_to_replace->size);
  const auto status = context->ReplaceNodeSubsetsWithDelegateKernels(
      context, kRegistration, op_to_replace, delegate);
  LOGI("Created %d GPU delegate kernels.",
                  gpu_delegate->num_delegate_kernels());
  TFLITE_LOG_PROD(TFLITE_LOG_INFO, "Created %d GPU delegate kernels.",
                  gpu_delegate->num_delegate_kernels());
  TfLiteIntArrayFree(op_to_replace);
  return status;
}
