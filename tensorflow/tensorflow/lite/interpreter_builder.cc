...
#define LOGV(...) __android_log_print(ANDROID_LOG_VERBOSE, "interpreter_builder", __VA_ARGS__) 
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG , "interpreter_builder", __VA_ARGS__)
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO  , "interpreter_builder", __VA_ARGS__)
#define LOGW(...) __android_log_print(ANDROID_LOG_WARN  , "interpreter_builder", __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR  , "interpreter_builder", __VA_ARGS__)
...
TfLiteStatus InterpreterBuilder::operator()(
  ...
  for (int subgraph_index = 0; subgraph_index < subgraphs->size();
    ...
    if (!operators || !tensors) {
      LOGI("mt---Did not get operators or tensors in subgraph %d.",subgraph_index);
      TF_LITE_REPORT_ERROR(error_reporter_,
